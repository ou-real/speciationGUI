...
//	LocalDateTime now = LocalDateTime.now(); 
	double[] dividingLines;
	//ArrayList<Double> counter;//this keeps track of the number of birds
	//with beaksizes above or below the dividingLines

	public Simulation(Bird.SelectionType selectiontype, Bird.InheritanceMethod method, Gene.Type genetype, 
			double[] means, double[] stDevs, String dateTime) throws IOException {
		...
		//somewhere here, we will send the seed distribution to mathematica
		//and have it send back the dividing lines
		
		//expects the file to be there
		new File("Output\\" + dateTime).mkdirs();
		dataFile = new File("Output\\"+ dateTime + "\\" + method + " " +
				selectiontype.toString() + " " + genetype + " " + 
				Arrays.toString(means) + " " + Arrays.toString(stDevs) + 
				" " + simnumber++ + ".csv");
		//dataFile.mkdirs();
		
		//dataFile = new File("Output.csv");
		...
		for(double d : stDevs) 
			writer.write(Double.toString(d) + ",");
		writer.write("\n");
		//writer.write(Arrays.toString(means) + "\n" + Arrays.toString(stDevs) + "\n");
	...
	public void countingMethod(ArrayList<Bird> birds) {
		
	}
	
	public String getFullFileName() {
		//need to return full fileName, not just fileName
		return dataFile.getAbsolutePath();
	}

	private ArrayList<Bird> genpop() {
	...
	private ArrayList<Bird> dryseason(ArrayList<Bird> birds) {
		...
		for(int i=0; i<numseeds; i++) { //P6
			...
			island[randx][randy].add(new Seed(means, stDevs)); //P3, PA, PK
		}
	...
	private ArrayList<Bird> matingseason(ArrayList<Bird> birds) {
	...
	private ArrayList<Bird> killoldbirds(ArrayList<Bird> birds) {
	...
	private static ArrayList<Bird> shuffle(ArrayList<Bird> array) {
